...
AnsiString GetRegValue(const AnsiString &Key, const AnsiString &ValueName, HKEY RootKey, const AnsiString &Default)
{
  ...
  if(RegOpenKeyExA(RootKey, Key.c_str(), 0, KEY_QUERY_VALUE, &RegKey) == ERROR_SUCCESS)
  {
    DWORD Size;
    DWORD Type;
    if(RegQueryValueExA(RegKey, ValueName.c_str(), NULL, &Type, NULL, &Size) == ERROR_SUCCESS && Type == REG_SZ)
    {
      std::vector<BYTE> Data(Size);
      if(RegQueryValueExA(RegKey, ValueName.c_str(), NULL, NULL, &Data[0], &Size) == ERROR_SUCCESS)
...
unsigned GetRegValue(const AnsiString &Key, const AnsiString &ValueName, HKEY RootKey, unsigned Default)
{
  ...
  {
    DWORD Type;
    RegQueryValueExA(RegKey, ValueName.c_str(), NULL, &Type, NULL, NULL);
    if(Type == REG_DWORD)
    {
      DWORD Data;
      DWORD Size = sizeof(Data);
      if(RegQueryValueExA(RegKey, ValueName.c_str(), NULL, NULL, reinterpret_cast<unsigned char*>(&Data), &Size) == ERROR_SUCCESS)
        Result = Data;
    }
    RegCloseKey(RegKey);
  }
