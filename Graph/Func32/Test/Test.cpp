...
//---------------------------------------------------------------------------
bool CompareFunc(const TFunc &f1, const TFunc &f2, double x, int SignificantDigits)
{
	ECalcError E1;
	ECalcError E2;
	...
	if(E1.ErrorCode == ecNoError)
		return ::IsEqual(y1, y2, SignificantDigits);
	return true;
}
//---------------------------------------------------------------------------
bool CompareFunc(const TFunc &f1, const TFunc &f2, int SignificantDigits=5)
{
	double x[] = {-1000, -100, -10, -1, 0, 1, 10, 100, 1000, -0.5, -0.01, 0.5, 0.01};
	for(unsigned I = 0; I < sizeof(x)/sizeof(x[0]); I++)
    if(!CompareFunc(f1, f2, x[I], SignificantDigits))
      return false;
  return true;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void TestDif(const std::wstring &Str, long double x, TErrorCode ErrorCode)
{
  TFunc Dif;
	try
	{
		TFunc Func(Str);
		Dif = Func.MakeDif();
    long double y_dif = Dif(x);
		wcerr << "f(x)=" << Str << std::endl;
		wcerr << "f'(x)=" << Dif.MakeText() << std::endl;
    wcerr << "x=" << x << "=> f'(x)=" << y_dif << std::endl;
		wcerr << "Expected error: " << ErrorCode << std::endl << std::endl;
	}
	catch(EFuncError &E)
	{
...
{
	//Test parsing errors
	TestError(L"sinx", 0, ecUnknownVar);
	TestError(L"x2", 0, ecUnknownVar);
	TestError(L"sin2 x", 0, ecUnknownVar);
	TestError(L"integrate(x, 5)", 0, ecArgCountError);
	TestError(L"integrate(x, x, 5, 7, 8)", 0, ecArgCountError);

	//Test redundant space
	Test(L"x*sin x", PI/2, PI/2);
	Test(L"  x*sin x", PI/2, PI/2);
	Test(L"x * sin x", PI/2, PI/2);
	Test(L"x*sin x   ", PI/2, PI/2);
	Test(L"x*sin    x", PI/2, PI/2);
	TestError(L"", 0, ecEmptyString);
	TestError(L"     ", 0, ecEmptyString);

	//Test case (in)sesitivity
	Test(L"Sin X+x/pI", 0, 0);
	TestError(L"4.5e2", 0, ecUnknownVar);
	TestError(L"4.5I", 0, ecUnknownVar);

	//Test numbers
	Test(L"45", 0, 45);
	Test(L"-0.5796", 0, -0.5796);
  Test(L"-.5796", 0, -0.5796);
	Test(L"-5.478E-23x", 10, -5.478E-22);
  Test(L"pi", 0, PI);
  Test(L"e", 0, EULER);
  TestError(L"E", 0, ecUnknownVar);
  TestEval<TComplex>(L"i*i", 0, -1);
	TestErrorEval<long double>(L"i*i", 0, ecComplexError);
	Test(L"1E400*x", 2, StrToDouble("2E400")); //2E400 doesn't work directly with BCC 5.6.4; Fails under Cygwin
	Test(L"1E4000", 1, StrToDouble("1E4000")); //2E4000 doesn't work directly with BCC 5.6.4; Fails under Cygwin
	TestError(L"1E5000", 1, ecInvalidNumber); //Number too large
  TestError(L"1.2.3", 0, ecInvalidNumber);
	TestError(L"5.", 0, ecInvalidNumber);
  ...
  TestError(L"round x", 1.2345, ecArgCountError);
	TestError(L"round(x)", 1.2345, ecArgCountError);
	Test(L"round(x, 0)", 1.2345, 1);
  TestError(L"sqrt(x", 0, ecNoEndPar);

	//Implied multiplication
  Test(L"10sin(pi/2)x", 10, 100);
	Test(L"sin(pi*x)4x", 1.5, -6);
  Test(L"-3x", 10, -30);

	//Test common operators
  Test(L"4.5+x", 3.1, 7.6);
  Test(L"x+4.5", 3.1, 7.6);
  Test(L"4.5-x", 3.1, 1.4);
  Test(L"x-4.5", 3.1, -1.4);
	Test(L"4.5*x", 3.1, 13.95);
	Test(L"x*4.5", 3.1, 13.95);
	Test(L"4.5/x", 3.1, 1.4516129);
  Test(L"x/4.5", 3.1, 0.688888889);
  Test(L"(x)-3", 1, -2);
  Test(L"-3*x", 10, -30);
  TestError(L"x(5.4)", 0, ecParAfterConst);
  TestError(L"rand(2)", 0, ecParAfterConst);
  Test(L"x 5.4", 3, 16.2);     //Should this be allowed?
	Test(L"--x", 5, 5);
	Test(L"-x", 5, -5);
  Test(L"-5", NaN, -5);
	Test(L"++x", 5, 5);
	...
	Test(L"-x^2", 10, -100);
	Test(L"(-x)^2", 10, 100);
	Test(L"x^x", 0, 1);         //Important test
	Test(L"x^0", 458.789, 1);   //Important test
	Test(L"0^x", 456.789, 0);   //Important test
	Test(L"log(x)^2", 100, 4);
	TestError(L"x/(x^2-4)", -2, ecDivByZero);
	...
	TestError(L"x^(-1)", 0, ecPowCalcError);
	TestErrorEval<TComplex>(L"0^x", TComplex(3,1), ecPowCalcError);
	TestEval<TComplex>(L"0^x", TComplex(3,0), 0);
	Test(L"e^(2x)", 2, M_E*M_E*M_E*M_E);
	Test(L"e^2x", 2, M_E*M_E*M_E*M_E); //Same as the above
	Test(L"x^2^3", 10, 1E8);
	Test(L"(x^2)^3", 10, 1E6);
	Test(L"3^x^2", 4, 43046721);
	TestErrorEval<TComplex>(L"0^x", TComplex(-2.8, 1), ecPowCalcError);
	TestError(L"x^-2.8", 0, ecPowCalcError);
	Test(L"e^x", -10000, 0);
	...
	TestError(L"x^(2/0)", -8, ecDivByZero);
	Test(L"x^(2/3)", 0, 0);
	Test(L"x^(4/2)", -2, 4);

	//Test trigonometry functions
	Test(L"sin(x)", PI/2, 1);
	Test(L"sin(x)", 90, 1, Degree);
	Test(L"cos(x)", PI/2, 0);
	Test(L"cos(x)", 90, 0, Degree);
	Test(L"tan(x)", PI/4, 1);
	Test(L"tan(x)", 45, 1, Degree);
	Test(L"sec(x)", PI, -1);
	Test(L"sec(x)", 180, -1, Degree);
  Test(L"csc(x)", PI/2, 1);
  Test(L"csc(x)", 90, 1, Degree);
  Test(L"cot(x)", PI/4, 1);
	Test(L"cot(x)", 45, 1, Degree);

  //Test inverse trigonometry functions
	Test(L"asin(x)", 1, PI/2);
	Test(L"asin(x)", 1, 90, Degree);
	Test(L"asin(x)", 0.4, 0.41151684606748801938473789761734);
	Test(L"asin(x)", 0.5, PI/6);
	...
	Test(L"atan(x)", 1, PI/4);
	Test(L"atan(x)", 0, 0);
	Test(L"atan(x)", 1, 45, Degree);
	TestEval<TComplex>(L"atan(x)", TComplex(2.5, 1), TComplex(1.233425856, 0.1236740605));

	Test(L"asec(x)", -1, PI);
	Test(L"asec(x)", -1, 180, Degree);
	Test(L"acsc(x)", 1, PI/2);
	Test(L"acsc(x)", 1, 90, Degree);
	Test(L"acot(x)", 1, PI/4);
	Test(L"acot(x)", 1, 45, Degree);
	Test(L"acot(x)", 0, PI/2);
	Test(L"acot(x)", 0, 90, Degree);

	//Test logarihms
	Test(L"log(x)", 10000, 4);
	Test(L"logb(x,2)", 32, 5);
	Test(L"ln(x)", EULER*EULER, 2);
	Test(L"exp(x)", 2, EULER*EULER);
  TestError(L"log(-x)", 0, ecLogError);

  //Test square and roots
  Test(L"sqr(x)", 5, 25);
  Test(L"sqrt(x)", 100, 10);
  Test(L"root(3,x)", 125, 5);
  ...
  Test(L"sinh(x)", 5, 74.20321058);
  Test(L"cosh(x)", 5, 74.20994852);
	Test(L"tanh(x)", 5, 0.999909204);

	Test(L"asinh(x)", 5, 2.31243834);
	Test(L"asinh(x)", 0, 0);
	TestEval<TComplex>(L"asinh(x)", TComplex(1.5, 0.7), TComplex(1.256224291, 0.3776375126));

	Test(L"acosh(x)", 5, 2.29243167);
	Test(L"acosh(x)", 1, 0);
	...
	Test(L"atanh(x)", 0.5, 0.54930614);
	Test(L"atanh(x)", 0, 0);
	TestError(L"atanh(x)", 1, ecATanhError);
	TestError(L"atanh(x)", -1, ecATanhError);
	TestEval<TComplex>(L"atanh(x)", TComplex(2.5, 1), TComplex(0.351335639, 1.415944855));

	Test(L"abs(x)", -4.67, 4.67);
  ...
  Test(L"re(x)", 3, 3);
	Test(L"im(x)", 3, 0);
  TestErrorEval<long double>(L"re(e^(i*x))", 1, ecComplexError);
  TestErrorEval<long double>(L"im(e^(i*x))", 1, ecComplexError);
  TestEval<TComplex>(L"re(e^(i*x))", 1, 0.5403023059);
  TestEval<TComplex>(L"im(e^(i*x))", 1, 0.8414709848);

  //Rounding
	Test(L"trunc(x)", -4.567, -4);
	Test(L"fract(x)", -4.567, -0.567);
  Test(L"ceil(x)", -4.567, -4);
  Test(L"floor(x)", -4.567, -5);
  Test(L"Round(x, 3)", 412.4572, 412.457);
  Test(L"Round(x, 2)", 412.4572, 412.46);
  Test(L"Round(x, 1)", 412.4572, 412.5);
  Test(L"Round(x, 0)", 412.4572, 412);
	Test(L"Round(x, -2)", 412.4572, 400);
  Test(L"Round(x, 2)", -412.4572, -412.46);

  //Boolean functions
  Test(L"not(x)", -4.567, 0);
  Test(L"not(x)", 0, 1);
	Test(L"x=5", 5, 1);
	Test(L"x=5", 3, 0);
...
//	Test("((1-x)^2)^.5+((1+2)^2)^.5 = ((x+2)^2)^.5", -5, 0);
  ...
  Test(L"0 xor 0", 0, 0);
	...
	Test(L"if(x<3, 5, 9)", 2, 5);
  Test(L"if(x<3, 5, 9)", 3, 9);
  Test(L"if(x<3, 5, 9)", 4, 9);
  Test(L"range(2,x,5)", 1, 2);
	Test(L"range(2,x,5)", 3, 3);
  Test(L"range(2,x,5)", 6, 5);
  Test(L"min(8, 2, 5)", 0, 2);
	Test(L"min(x,2)", 1, 1);
  Test(L"max(8, 2, 5)", 0, 8);

	Test(L"gamma(x)", 6, 120);
  Test(L"mod(x,5)", 12, 2);
  Test(L"mod(x,5)", 10, 0);
  Test(L"mod(x,-1)", -2, 0);
  Test(L"mod(x,-5)", 12, -3);
  Test(L"mod(x,-5)", -12, -2);
  Test(L"mod(x, 1.2)", 5.6, 0.8);
  Test(L"mod(x, 1.2)", -5.6, 0.4);
  Test(L"dnorm(x, 5, 7)", 3, 0.054712394277745);
  Test(L"dnorm(x)", 3, 0.00443184841193801);
	Test(L"dnorm(x, 0, 1)", 3, 0.00443184841193801);

	//Test backward compatibility
  Test(L"integrate(x^2,2,5)", NaN, 39);
	TestError(L"sum(x, 3, 7)", NaN, ecArgCountError);
  TestError(L"product(x, 3, 7)", NaN, ecArgCountError);

	//Test improved integrate
	Test(L"integrate(x^2,x,2,5)", NaN, 39);
  Test(L"integrate(dnorm(x,100,60),x,-inf,100)", 0, 0.5);
  Test(L"integrate(dnorm(x,100,20),x,-inf,100)", 0, 0.5);
  Test(L"integrate(dnorm(x,100,60),x,100,inf)", 0, 0.5);
  Test(L"integrate(dnorm(x,100,20),x,100,+inf)", 0, 0.5);
	Test(L"integrate(e^x,x,-inf,0)", 0, 1);
	Test(L"integrate(e^x,x,0,-inf)", 0, -1);
	Test(L"integrate(e^-x,x,inf,0)", 0, -1);
	Test(L"integrate(e^-x,x,0,inf)", 0, 1);
	Test(L"integrate(e^-abs(x),x,-inf,inf)", 0, 2);
	Test(L"integrate(e^-abs(x),x,inf,-inf)", 0, -2);
	Test(L"integrate(x*t^2, t, 0, 3)", 2, 18);
	Test(L"sum(x, x, 3, 7)", NaN, 3+4+5+6+7);
	Test(L"product(x, x, 3, 7)", NaN, 3*4*5*6*7);
	Test(L"sum(x*t, t, 3, 7)", 2, 2*3+2*4+2*5+2*6+2*7);
	Test(L"product(x*t, t, 3, 7)", 2, 2*3*2*4*2*5*2*6*2*7);
	Test(L"sum(min(k,8),k,0,x)", 10, 52);
	Test(L"sum(min(k,8)*2,k,0,x)", 10, 104);
	Test(L"sum(min((k),[6+2])*(2)/2,k,0,x)", 10, 52);
	Test(L"sum(sum(k*l,k,1,10),l,1,x)", 2, 165);
	Test(L"integrate(1/((1+x)*x^(1/2)),x, 0, inf)", 0, PI);
	TestEval<long double>(L"integrate(1/x^(2/3),-1,1)", 0, 6);
	TestErrorEval<TComplex>(L"integrate(1/x^(2/3),-1,1)", 0, ecNoResult);
  TestError(L"integrate(t, t, 4test, x)", 0, EFuncError(ecUnknownVar, L"test"));
  TestError(L"integrate(t, t, x, 4test)", 0, EFuncError(ecUnknownVar, L"test"));
...
//	TestError("sum(x,1,0,10)", 0, ecLiteralExpected);
  ...
  //Test undef
	TestError(L"undef", 0, ecNotDefError);
	Test(L"if(x<5, x, undef)", 0, 0);
	TestError(L"if(x<5, x, undef)", 6, ecNotDefError);

  //Test block function
	Test(L"if(x<-2, x, 2x)", -3, -3);
  Test(L"if(x<-2, x, 2x)", 1, 2);
  Test(L"if(x<-2, x, x<3, 2x, 3x)", 1, 2);
  Test(L"if(x<-2, x, x<3, 2x, 3x)", 4, 12);
  Test(L"2+if(x<-2, x, x<3, 2x, 3x)", 4, 14);
  Test(L"if(x<-2, x, x<3, 2x)", 1, 2);
	TestError(L"if(x<-2, x, x<3, 2x)", 4, ecNotDefError);

  //Test Zeta function
  Test(L"zeta(x)", 0, -0.5);
  Test(L"zeta(x)", 10, 1.000994575);
  TestError(L"zeta(x)", 1, ecNotDefError);
  Test(L"zeta(x)", -5, -1.0/252);
  ...
  Test(L"W(-1/e)", 0, -1);
	Test(L"W(x)", 0, 0);
  Test(L"W(x)", -M_LN2/2, -M_LN2);
  Test(L"W(x)", 1, 0.5671432904097838729999686622);
	Test(L"W(x)", M_E, 1);

	//Test handling of other symbols
	Test(L"80 \x2212 2*x", 5, 70); //Test Minus sign (0x2212)
	Test(L"\x2212 5x", 2, -10); //Test Minus sign (0x2212)
	TestError(L"?2", 0, ecUnknownChar);
	TestError(L"x+?", 0, ecUnknownChar);
	TestError(L"x+sin ¤", 0, ecUnknownChar);
	TestError(L"x * &", 0, ecUnknownChar);
	...
	Test(L"[x]+5", 3, 8);
	Test(L"log[x]+5", 10, 6);
	Test(L"log[x+20] * 3", 80, 6);
	Test(L"log[10] x", 10, 10);
	...
	Test(L"2+(x*[5+x*{2-x}])", 10, -748);
	Test(L"log[log(log{x}10)10]", 10, 1);
	TestError(L"2+(x*[5+x*{2-x})]", 10, ecNoEndPar);
	TestError(L"log[log(log{x}10]10)", 10, ecNoEndPar);

  //Test relations
  Test(L"sin(x^2) < cos(x^2)", 0, 1);

	//Test difficult function
	Test(L"-sin(x)+(-sin(x)*sqrt((-sin(x))^2+cos(x)^2)-cos(x)*0.5/sqrt((-sin(x))^2+cos(x)^2)*(--2*sin(x)*cos(x)-2*cos(x)*sin(x)))/sqrt((-sin(x))^2+cos(x)^2)^2", 0, 0);
  ...
  TestDif(L"-0.5796", L"0");
  TestDif(L"-3x", L"-3");
  TestDif(L"-3x^1", L"-3");
  TestDif(L"-3x^2", L"-6x");
  TestDif(L"-3x^3", L"-9x^2");
	TestDif(L"5x+sin(x)", L"5+cos(x)");
  TestDif(L"5x-sin(x)", L"5-cos(x)");
  TestDif(L"5/x", L"-5/x^2");
  TestDif(L"sin(x)/x", L"(cos(x)*x-sin(x))/x^2");
  TestDif(L"2*sin 30*x", L"1", Degree);
  TestDif(L"x-cos x", L"1+sin x");
  TestDif(L"e^-(sqrt(2)*x)",  L"-e^-(sqrt(2)*x)*sqrt(2)");

  //Test differentiation of special functions
  TestDif(L"dnorm(x)", L"-exp(-x^2/2)*x*sqrt(2pi)/2pi");
	TestDif(L"dnorm(x, 0, 1)", L"-exp(-x^2/2)*x*sqrt(2pi)/2pi");
	TestDif(L"mod(sin(x), 5)", L"cos(x)");
	TestDif(L"mod(4, x)", ecNotDifAble);

  //Test differentiation of inverse trigonometric functions
  TestDif(L"acot(x)", 1, -28.647889756541160438399077407053, Degree);

  //Differentiation of special power handling
  TestDif(L"x^(1/3)", L"1/3*x^(-2/3)");
  TestDif(L"2^(x/3)", L"1/3*ln(2)*2^(x/3)");
  TestDif(L"2^(1/x)", L"-ln(2)*2^(1/x)/x^2");

	//Differentiation of special functions
	TestDif(L"sum(k*x, k, 1, 10)", L"sum(k, k, 1, 10)");
	TestDif(L"sum(sum(k*l,k,1,10),l,1,x)", L"0");
	TestDif(L"sum(sum(k*l*x,k,1,10),l,1,2)", L"sum(sum(k*l,k,1,10),l,1,2)");
	TestDif(L"product(k*x, k, 1, 10)", ecNotDifAble);
	TestDif(L"integrate(k*x, k, 1, 10)", L"integrate(k,k,1,10)");
	TestDif(L"integrate(x, x, 1, 10)", L"0");
	TestDif(L"integrate(x, 1, 10)", L"0");
	TestDif(L"integrate(sin(x), x, 0, x)", L"sin(x)");
	TestDif(L"integrate(sin(x), 0, x)", L"sin(x)");
	TestDif(L"fact(x)", ecNotDifAble);
	TestDif(L"fact(5)", L"0");

	//Differentiation of piecewise functions
	TestDif(L"min(x,2)", L"if(x<2,1,0)");
  ...
  TestDif(L"fract(x^2)", L"2x");
  TestDif(L"sqrt(fract(x)^2)", L"0.5/sqrt(fract(x)^2)*2*fract(x)");
  TestDif(L"sign(x)", -1, 0);
  TestDif(L"sign(x)", 1, 0);
  TestDif(L"sign(x)", 0.0L, ecNotDefError);
  TestDif(L"sign(x)", L"if(x,0)");
  TestDif(L"u(x)", -1, 0);
  TestDif(L"u(x)", 1, 0);
  TestDif(L"u(x)", 0, INF);

  //Test the simplify code
	TestSimplify(L"ln(e)", L"1");
	TestSimplify(L"log(10)", L"1");
	TestSimplify(L"1--sin x", L"1+sin(x)");
	TestSimplify(L"--sin x", L"sin(x)");
	TestSimplify(L"sqrt(x^2)", L"sqrt(x^2)"); //Warning: Do not simplify to "x". It is not the same
	TestSimplify(L"sqrt(x)^2", L"sqrt(x)^2"); //Warning: Do not simplify to "x". It is not the same
	TestSimplify(L"(-x)^2", L"x^2");
	TestSimplify(L"-sin(x)+(-sin(x)*sqrt((-sin(x))^2+cos(x)^2)-cos(x)*0.5/sqrt((-sin(x))^2+cos(x)^2)*(--2*sin(x)*cos(x)-2*cos(x)*sin(x)))/sqrt((-sin(x))^2+cos(x)^2)^2",
							 L"-sin(x)+(-sin(x)*sqrt(sin(x)^2+cos(x)^2)-0.5*cos(x)/sqrt(sin(x)^2+cos(x)^2)*(2*sin(x)*cos(x)-2*cos(x)*sin(x)))/sqrt(sin(x)^2+cos(x)^2)^2");
  ...
  SymbolList.Add(L"foo_bar", 5);
	Test(L"k*x", 2, 84, Radian, L"", SymbolList);
	Test(L"TestSqr(x)", 10, 100, Radian, L"", SymbolList);
	Test(L"TestCube(5, 2, x)", 1, 130, Radian, L"", SymbolList);
	Test(L"foo(5, 2, x)", 1, 130, Radian, L"", SymbolList);
  Test(L"foo_bar", 0, 5, Radian, L"", SymbolList);
